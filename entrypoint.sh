...
if [ "$FUZZER_MODE" = "master" ]; then
    ...
    python3 -u /app/fuzz.py /app/config_docker.txt -M "$INSTANCE_NAME" -o /app/fume_sync 2>&1
elif [ "$FUZZER_MODE" = "secondary" ]; then
    echo "[*] Running as SECONDARY instance: $INSTANCE_NAME"
    python3 -u /app/fuzz.py /app/config_docker.txt -S "$INSTANCE_NAME" -o /app/fume_sync 2>&1
else
    echo "[*] Running in STANDALONE mode"
    python3 -u /app/fuzz.py /app/config_docker.txt -o /app/fume_sync 2>&1
fi
