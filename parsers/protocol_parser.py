class ProtocolParser:
    ...
    def insertStringPair(
        self, fieldName, payload, index, key_Use_G_field, value_Use_G_field
    ):
        index = self.insertString(fieldName + " key", payload, index, key_Use_G_field)
        index = self.insertString(
            fieldName + " value", payload, index - 2, value_Use_G_field
        )
        return index

    def insertVariableByteIntegerNoIdentifier(
        self, fieldName, payload, index, use_G_field
    ):
        return self.insertVariableByteInteger(
            fieldName, payload, index - 2, use_G_field
        )
    ...
    def parsePropertiesHelper(self, properties):
        ...
        while index < len(properties):
            if self.indexToByte(index, 1, properties) == "01":
                index = self.insertByte(
                    "payload format indicator", properties, index, True
                )

            if self.indexToByte(index, 1, properties) == "02":
                index = self.insertFourBytes(
                    "message expiry interval", properties, index, False
                )
            ...
            if self.indexToByte(index, 1, properties) == "09":
                index = self.insertBinaryData(
                    "correlation data", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "0b":
                index = self.insertVariableByteInteger(
                    "subscription identifier", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "11":
                index = self.insertFourBytes(
                    "session expiry interval", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "12":
                index = self.insertString(
                    "assigned client identifier", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "13":
                index = self.insertTwoBytes(
                    "server keep alive", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "15":
                index = self.insertString(
                    "authentication method", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "16":
                index = self.insertBinaryData(
                    "authentication data", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "17":
                index = self.insertByte(
                    "request problem information", properties, index, True
                )

            if self.indexToByte(index, 1, properties) == "18":
                index = self.insertFourBytes(
                    "will delay interval", properties, index, False
                )

            if self.indexToByte(index, 1, properties) == "19":
                index = self.insertByte(
                    "request response information", properties, index, True
                )

            if self.indexToByte(index, 1, properties) == "1a":
                index = self.insertString(
                    "response information", properties, index, True
                )
            ...
            if self.indexToByte(index, 1, properties) == "22":
                index = self.insertTwoBytes(
                    "topic alias maximum", properties, index, True
                )
            ...
            if self.indexToByte(index, 1, properties) == "26":
                index = self.insertStringPair(
                    "user property", properties, index, False, False
                )

            if self.indexToByte(index, 1, properties) == "27":
                index = self.insertFourBytes(
                    "maximum packet size", properties, index, True
                )

            if self.indexToByte(index, 1, properties) == "28":
                index = self.insertByte(
                    "wildcard subscription available", properties, index, True
                )

            if self.indexToByte(index, 1, properties) == "29":
                index = self.insertByte(
                    "subscription identifier available", properties, index, True
                )

            if self.indexToByte(index, 1, properties) == "2a":
                index = self.insertByte(
                    "shared subscription available", properties, index, True
                )
