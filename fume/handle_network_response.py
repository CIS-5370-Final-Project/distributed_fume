...
def handle_network_response(recv):
    ...
    while index < len(recv.hex()):
        try:
            ...
            if g.USE_REDIS:
                if not g.redis_conn.exists(G_fields):
                    # Save the payload that caused this new response
                    g.redis_conn.set(G_fields, g.payload)
                    pv.normal_print("Worker %d found new network response!" % g.WORKER_ID)
            else:
