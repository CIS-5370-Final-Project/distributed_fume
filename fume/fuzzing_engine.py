...
def handle_response_log_state(mm):
    ...
    if response_type_pick == 0:
        if g.USE_REDIS:
            random_key = g.redis_conn.randomkey()
            if random_key:
                g.payload = bytearray(g.redis_conn.get(random_key))
            else:
                # Fallback if Redis is empty or connection fails
                mm.current_state = mm.state_connect
                handle_state(mm)
        elif len(g.network_response_log) > 0:
