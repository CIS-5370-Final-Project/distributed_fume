...
class Properties(Packet):
    ...
    def __init__(self, whitelist=None):
        ...
        self.subscription_identifier = "0b" + self.toVariableByte(
            "%x" % self.subscription_identifier_value
        )
        ...
        self.assigned_client_identifier = self.toEncodedString(
            0x12, self.assigned_client_identifier_length
        )
        ...
        self.authentication_method = self.toEncodedString(
            0x15, self.authentication_method_len
        )
        self.conditionalAppend(whitelist, 0x15, self.authentication_method)

        self.authentication_data_len = random.randint(0, 30)
        self.authentication_data = self.toEncodedString(
            0x16, self.authentication_data_len
        )
        ...
        self.response_information = self.toEncodedString(
            0x1A, self.response_information_length
        )
        ...
        self.user_property = self.toEncodedStringPair(
            0x26, self.user_property_name_length, self.user_property_value_length
        )
